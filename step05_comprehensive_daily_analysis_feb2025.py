...
def get_current_close_price(conn, symbol, trade_date):
    """Get current close price from step03 table"""
    query = """
    SELECT close_price
    ...
    try:
        result = conn.execute(query, symbol, trade_date).fetchone()
        if result:
            return float(result[0])
        else:
            # Fallback to step04 data if not found in step03
            fallback_query = """
...
def process_single_date_symbol(conn, trade_date, symbol):
    ...
    try:
        ...
        if current_price is None:
            logger.warning(f"⚠️ No current price found for {symbol} on {trade_date}")
            return []
        
        # Get nearest strikes
        strikes = get_nearest_strikes_for_symbol_date(conn, symbol, trade_date, current_price)
        if not strikes:
            logger.warning(f"⚠️ No strikes found for {symbol} on {trade_date}")
            return []
        
        # Get option data for these strikes
        option_data = get_option_data_for_strikes(conn, symbol, trade_date, strikes)
        if not option_data:
            logger.warning(f"⚠️ No option data found for {symbol} on {trade_date}")
            return []
        ...
        logger.info(f"✅ Processed {symbol} on {trade_date}: {len(records)} records")
        return records
...
def main():
    ...
    try:
        ...
        if not trading_dates:
            logger.error("❌ No trading dates found for February 2025")
            return
        ...
        for i, trade_date in enumerate(trading_dates, 1):
            ...
            logger.info(f"\n📅 Processing {date_str} ({i}/{len(trading_dates)})")
            ...
            for j, symbol in enumerate(symbols, 1):
                if j % 20 == 0:  # Progress update every 20 symbols
            ...
            if date_records:
                ...
                logger.info(f"✅ {date_str}: Inserted {inserted_count} records")
            else:
                logger.warning(f"⚠️ {date_str}: No records to insert")
