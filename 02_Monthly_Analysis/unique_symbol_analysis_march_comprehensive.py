#!/usr/bin/env python3
"""
🚀 NSE MARCH 2025 Unique Symbol Analysis - No Duplicates
...
- Matches July/January/February 2025 analysis format exactly
"""
...
def create_unique_symbol_analysis_march():
    """Create unique symbol analysis from March 2025 CSV files"""
    
    print("📊 NSE MARCH 2025 Unique Symbol Analysis - No Duplicates")
    print("=" * 60)
    
    # Check for March 2025 CSV files
    march_csv_pattern = "NSE_March_2025_Data/cm*.csv"
    csv_files = glob.glob(march_csv_pattern)
    
    if not csv_files:
        print("❌ No March 2025 CSV files found!")
        print(f"💡 Looking for files matching: {march_csv_pattern}")
        return
    
    print(f"📁 Found {len(csv_files)} CSV files from March 2025:")
    ...
    filename = f"NSE_MARCH2025_data_Unique_Symbol_Analysis_{timestamp}.xlsx"
    ...
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            # Analysis summary
            summary_data.append(['📊 MARCH 2025 ANALYSIS SUMMARY', ''])
            summary_data.append(['', ''])
...
if __name__ == "__main__":
    create_unique_symbol_analysis_march()
