#!/usr/bin/env python3
"""
🚀 NSE FEBRUARY 2025 Unique Symbol Analysis - No Duplicates
...
- Matches July/January 2025 analysis format exactly
"""
...
def create_unique_symbol_analysis_february():
    """Create unique symbol analysis from February 2025 CSV files"""
    
    print("📊 NSE FEBRUARY 2025 Unique Symbol Analysis - No Duplicates")
    print("=" * 60)
    
    # Check for February 2025 CSV files
    february_csv_pattern = "NSE_February_2025_Data/cm*.csv"
    csv_files = glob.glob(february_csv_pattern)
    
    if not csv_files:
        print("❌ No February 2025 CSV files found!")
        print(f"💡 Looking for files matching: {february_csv_pattern}")
        return
    
    print(f"📁 Found {len(csv_files)} CSV files from February 2025:")
    ...
    filename = f"NSE_FEBRUARY2025_data_Unique_Symbol_Analysis_{timestamp}.xlsx"
    ...
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            # Analysis summary
            summary_data.append(['📊 FEBRUARY 2025 ANALYSIS SUMMARY', ''])
            summary_data.append(['', ''])
...
if __name__ == "__main__":
    create_unique_symbol_analysis_february()
