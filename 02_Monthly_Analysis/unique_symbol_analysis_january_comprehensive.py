#!/usr/bin/env python3
"""
🚀 NSE JANUARY 2025 Unique Symbol Analysis - No Duplicates
...
- Matches July 2025 analysis format exactly
"""
...
def create_unique_symbol_analysis_january():
    """Create unique symbol analysis from January 2025 CSV files"""
    
    print("📊 NSE JANUARY 2025 Unique Symbol Analysis - No Duplicates")
    print("=" * 60)
    
    # Check for January 2025 CSV files
    january_csv_pattern = "NSE_January_2025_Data/cm*.csv"
    csv_files = glob.glob(january_csv_pattern)
    
    if not csv_files:
        print("❌ No January 2025 CSV files found!")
        print(f"💡 Looking for files matching: {january_csv_pattern}")
        return
    
    print(f"📁 Found {len(csv_files)} CSV files from January 2025:")
    ...
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            # Analysis summary
            summary_data.append(['📊 JANUARY 2025 ANALYSIS SUMMARY', ''])
            summary_data.append(['', ''])
...
if __name__ == "__main__":
    create_unique_symbol_analysis_january()
