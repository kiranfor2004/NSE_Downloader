#!/usr/bin/env python3
"""
🚀 NSE MAY 2025 Unique Symbol Analysis - No Duplicates
...
- Matches July/January/February/March/April 2025 analysis format exactly
"""
...
def create_unique_symbol_analysis_may():
    """Create unique symbol analysis from May 2025 CSV files"""
    
    print("📊 NSE MAY 2025 Unique Symbol Analysis - No Duplicates")
    print("=" * 60)
    
    # Check for May 2025 CSV files
    may_csv_pattern = "NSE_May_2025_Data/cm*.csv"
    csv_files = glob.glob(may_csv_pattern)
    
    if not csv_files:
        print("❌ No May 2025 CSV files found!")
        print(f"💡 Looking for files matching: {may_csv_pattern}")
        return
    
    print(f"📁 Found {len(csv_files)} CSV files from May 2025:")
    ...
    filename = f"NSE_MAY2025_data_Unique_Symbol_Analysis_{timestamp}.xlsx"
    ...
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            # Analysis summary
            summary_data.append(['📊 MAY 2025 ANALYSIS SUMMARY', ''])
            summary_data.append(['', ''])
...
if __name__ == "__main__":
    create_unique_symbol_analysis_may()
