#!/usr/bin/env python3
"""
🚀 NSE JULY 2025 Unique Symbol Analysis - No Duplicates
...
def create_unique_symbol_analysis_july():
    """Create unique symbol analysis from July 2025 CSV files"""
    
    print("📊 NSE JULY 2025 Unique Symbol Analysis - No Duplicates")
    print("=" * 60)
    
    # Check for July 2025 CSV files
    july_csv_pattern = "NSE_July_2025_Data/sec_bhavdata_full_*.csv"
    csv_files = glob.glob(july_csv_pattern)
    
    if not csv_files:
        print("❌ No July 2025 CSV files found!")
        print(f"💡 Looking for files matching: {july_csv_pattern}")
        return
    
    print(f"📁 Found {len(csv_files)} CSV files from July 2025:")
    ...
    filename = f"NSE_JULY2025_data_Unique_Symbol_Analysis_{timestamp}.xlsx"
    ...
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            # Analysis summary
            summary_data.append(['📊 JULY 2025 ANALYSIS SUMMARY', ''])
            summary_data.append(['', ''])
...
if __name__ == "__main__":
    create_unique_symbol_analysis_july()
