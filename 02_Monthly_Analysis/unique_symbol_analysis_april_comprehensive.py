#!/usr/bin/env python3
"""
🚀 NSE APRIL 2025 Unique Symbol Analysis - No Duplicates
...
- Matches July/January/February/March 2025 analysis format exactly
"""
...
def create_unique_symbol_analysis_april():
    """Create unique symbol analysis from April 2025 CSV files"""
    
    print("📊 NSE APRIL 2025 Unique Symbol Analysis - No Duplicates")
    print("=" * 60)
    
    # Check for April 2025 CSV files
    april_csv_pattern = "NSE_April_2025_Data/cm*.csv"
    csv_files = glob.glob(april_csv_pattern)
    
    if not csv_files:
        print("❌ No April 2025 CSV files found!")
        print(f"💡 Looking for files matching: {april_csv_pattern}")
        return
    
    print(f"📁 Found {len(csv_files)} CSV files from April 2025:")
    ...
    filename = f"NSE_APRIL2025_data_Unique_Symbol_Analysis_{timestamp}.xlsx"
    ...
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            # Analysis summary
            summary_data.append(['📊 APRIL 2025 ANALYSIS SUMMARY', ''])
            summary_data.append(['', ''])
...
if __name__ == "__main__":
    create_unique_symbol_analysis_april()
