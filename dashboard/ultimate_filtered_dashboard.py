    ...
    def connect_and_fetch_data(self):
        ...
        try:
            ...
            SELECT 
                ...
                category,
                -- Calculated Fields
                CAST(((current_close_price - previous_close_price) / previous_close_price) * 100 AS DECIMAL(10,2)) as price_change_pct,
                CAST(((current_high_price - current_low_price) / current_close_price) * 100 AS DECIMAL(10,2)) as volatility_pct,
                CAST(((current_ttl_trd_qnty - previous_ttl_trd_qnty) / NULLIF(previous_ttl_trd_qnty, 0)) * 100 AS DECIMAL(10,2)) as volume_change_pct,
                CASE 
        ...
        <div class="filter-grid">
            ...
            <div class="filter-group">
                ...
                <select class="filter-input" id="index-filter" onchange="applyFilters()">
                    ...
                    {"".join([f'<option value="{idx}">{idx}</option>' for idx in filter_options['index_names']])}
                </select>
            </div>
            ...
            <div class="filter-group">
                ...
                <select class="filter-input" id="category-filter" onchange="applyFilters()">
                    ...
                    {"".join([f'<option value="{cat}">{cat}</option>' for cat in filter_options['categories']])}
                </select>
            </div>
            ...
            <div class="filter-group">
                ...
                <select class="filter-input" id="price-category-filter" onchange="applyFilters()">
                    ...
                    {"".join([f'<option value="{cat}">{cat}</option>' for cat in filter_options['price_categories']])}
                </select>
            </div>
            ...
            <div class="filter-group">
                ...
                <select class="filter-input" id="volume-category-filter" onchange="applyFilters()">
                    ...
                    {"".join([f'<option value="{cat}">{cat}</option>' for cat in filter_options['volume_categories']])}
                </select>
            </div>
            ...
            <div class="filter-group">
                ...
                <select class="filter-input" id="delivery-trend-filter" onchange="applyFilters()">
                    ...
                    {"".join([f'<option value="{trend}">{trend}</option>' for trend in filter_options['delivery_trends']])}
                </select>
            </div>
    ...
    <div class="results-panel" data-aos="fade-up" data-aos-delay="200">
        ...
        <div class="charts-grid">
            ...
            <div class="chart-container">
                ...
                <div id="performers-chart" style="height: 300px;"></div>
            </div>
            <div class="chart-container">
                <div class="chart-title">Market Heatmap</div>
        ...
        <div class="data-table">
            <div class="table-header">
                <i class="fas fa-table"></i> Filtered Results
            </div>
    ...
    <script>
        ...
        function applyQuickFilter(type) {{
            ...
            switch(type) {{
                ...
                case 'topVolume':
                    // Top 10% by volume
                    const volumeThreshold = originalData.map(d => d.current_ttl_trd_qnty).sort((a,b) => b-a)[Math.floor(originalData.length * 0.1)];
                    filteredData = originalData.filter(item => 
                        item.current_ttl_trd_qnty >= volumeThreshold
                    );
                    break;
                ...
                case 'topDelivery':
                    // Delivery > 70%
                    filteredData = originalData.filter(item => 
                        item.current_deliv_per > 70
                    );
                    break;
                    
                case 'momentum':
                    // High delivery increase + volume surge
                    filteredData = originalData.filter(item => 
                        item.delivery_increase_pct > 50 && 
                        item.volume_change_pct > 25
                    );
                    break;
                    
                case 'institutional':
                    // High delivery + high turnover + major index
                    filteredData = originalData.filter(item => 
                        item.current_deliv_per > 60 && 
                        item.current_turnover_lacs > 500 &&
                ...
                case 'value':
                    ...
                    filteredData = originalData.filter(item => 
                        item.current_close_price < 200 && 
                        item.delivery_increase_pct > 25 && 
                        item.current_deliv_per > 40
                    );
                    break;
            }}
        ...
        function applyFilters() {{
            ...
            data = data.filter(item => 
                item.current_close_price >= priceMin && 
                item.current_close_price <= priceMax
            );
            ...
            data = data.filter(item => 
                item.current_ttl_trd_qnty >= volumeMin && 
                item.current_ttl_trd_qnty <= volumeMax
            );
            ...
            data = data.filter(item => 
                item.current_deliv_per >= deliveryMin && 
                item.current_deliv_per <= deliveryMax
            );
        ...
        function updatePriceChart() {{
            ...
            const priceData = priceRanges.map(range => {{
                const count = filteredData.filter(item => 
                    item.current_close_price >= range.min && 
                    item.current_close_price < range.max
                ).length;
        ...
        function updateDeliveryChart() {{
            ...
            const chart = new ApexCharts(document.querySelector("#delivery-chart"), options);
            chart.render();
        }}
        ...
        function updateIndexChart() {{
            ...
            const chartData = Object.entries(indexData).map(([index, data]) => ({{
                category: index,
                count: data.count,
            ...
            const chart = new ApexCharts(document.querySelector("#index-chart"), options);
            chart.render();
        }}
        ...
        function updatePerformersChart() {{
            const topPerformers = filteredData
                .filter(item => item.price_change_pct !== null)
            ...
            const options = {{
                ...
                series: [{{ 
                    ...
                    data: topPerformers.map(item => ({{
                        x: item.symbol,
                        y: parseFloat(item.price_change_pct || 0)
                    }}))
                }}],
                colors: ['#48bb78'],
                xaxis: {{ 
            ...
            const chart = new ApexCharts(document.querySelector("#performers-chart"), options);
            chart.render();
        }}

        // Market Heatmap
        function updateHeatmapChart() {{
            const heatmapData = filteredData.slice(0, 100).map(item => [
                item.symbol,
                parseFloat(item.current_deliv_per || 0),
                parseFloat(item.price_change_pct || 0),
                parseInt(item.current_ttl_trd_qnty || 0)
            ]);
            
            Plotly.newPlot('heatmap-chart', [{{
                z: heatmapData.map(d => d[1]),
                x: heatmapData.map(d => d[0]),
                y: ['Delivery %'],
                type: 'heatmap',
                colorscale: 'Viridis'
            }}], {{
